...
static int vb__socket_valid(vb_socket_t socket)
{
	return socket > 0;
}

static void vb__socket_close(vb_socket_t socket)
{
	close(socket);
}
...
static int vb__socket_set_blocking(vb_socket_t socket, int blocking)
{
